const PROJECTS = {
  ...
  lucent: {
    ...
    `,
    links: [
      ["GitHub profile", "https://github.com/sushxnthd"]
    ]
  },
  calibration: {
    title: "Calibration",
...
function activeItem() {
  return items.find((item) => item.dataset.project === activeId) || visibleItems()[0];
}

function projectOf(id = activeId) {
  return PROJECTS[id];
}

function setMode(nextMode, targetId) {
  mode = nextMode;
  document.querySelectorAll(".nav-tab[data-mode]").forEach((button) => {
  ...
  const desired = candidates.find((item) => item.dataset.project === targetId) || candidates[0];
  if (desired) {
    activeId = desired.dataset.project;
    updateActive(desired, false);
    requestAnimationFrame(() => centerItem(desired, false));
  }
  document.getElementById("modeReadout").textContent = mode.toUpperCase();
}

function updateActive(item, center = false) {
...
function centerItem(item, smooth = true) {
  ...
  rail.scrollTo({ left: target, behavior: smooth && !reducedMotion ? "smooth" : "auto" });
}
...
function detectNearest() {
  ...
  scrollFrame = requestAnimationFrame(() => {
    ...
    if (nearest && nearest.dataset.project !== activeId) updateActive(nearest, false);
  });
}
...
function openProject(id = activeId) {
  ...
  document.getElementById("windowContent").innerHTML = project.window;
  ...
  document.getElementById("takeoverFooter").textContent = `SUSHANTH.DASARI / ${project.code}`;
...
function stepProject(direction) {
  ...
  let index = all.findIndex((item) => item.dataset.project === activeId);
  if (index < 0) index = 0;
...
function closeIndex() {
  ...
  if (!takeover.classList.contains("is-open")) document.body.style.overflow = "";
}

function jumpTo(id) {
  const item = items.find((candidate) => candidate.dataset.project === id);
  if (!item) return;
  if (item.dataset.kind !== mode) setMode(item.dataset.kind, id);
  else {
    updateActive(item, false);
    requestAnimationFrame(() => centerItem(item, true));
  }
  closeIndex();
}
...
function finishDrag(event) {
  ...
  setTimeout(() => centerItem(activeItem(), true), 20);
}
...
items.forEach((item) => {
  item.querySelector(".disc-button").addEventListener("click", (event) => {
    if (Math.abs(event.clientX - dragStartX) > 8 && dragging) return;
    updateActive(item, true);
    setTimeout(() => openProject(item.dataset.project), reducedMotion ? 0 : 220);
  });
...
window.addEventListener("keydown", (event) => {
  if (event.key === "Escape") {
    if (takeover.classList.contains("is-open")) closeProject();
...
window.addEventListener("resize", () => centerItem(activeItem(), false));
